...
powerArmaTierra::powerArmaTierra(vector3d& pos){
    settipo(tipoArmaTierra);
    setPower(4);
    setPosition(vector3d(pos.X,pos.Y,pos.Z));   
    getImage().initPowerTierra(getposition(),getHitBox());   
    setHitBox(vector3d(HitBoxPowerUp,HitBoxPowerUp,HitBoxPowerUp)) ;
    initPuntos();
}

void powerArmaTierra::efectoPowerUp(entity &enti){
   if(enti.isPlayer()){
